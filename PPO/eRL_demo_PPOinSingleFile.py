...
class ActorPPO(nn.Module):
    ...
    def __init__(self, mid_dim, state_dim, action_dim):
        ...
        self.a_logstd = nn.Parameter(torch.zeros((1, action_dim)) - 0.5,
                                     requires_grad=True)
    ...
    def get_logprob_entropy(self, state, action):
        ...
        logprob = -(self.a_logstd + self.sqrt_2pi_log + delta).sum(
            1)  # new_logprob
    ...
    def get_old_logprob(self, _action, noise):  # noise = action - a_noise
        ...
        return -(self.a_logstd + self.sqrt_2pi_log + delta).sum(
            1)  # old_logprob
...
class AgentPPO:
    ...
    def init(self,
        ...
        self.cri = self.ClassCri(net_dim, state_dim,
    ...
    def select_action(self, state):
        ...
        return actions[0].detach().cpu().numpy(), noises[0].detach().cpu(
        ).numpy()
    ...
    def explore_env(self, env, target_step):
        ...
        self.state = state
        '''splice list'''
        trajectory_list = self.trajectory_list + trajectory_temp[:last_done +
                                                                 1]
    ...
    def update_net(self, buffer, batch_size, repeat_times, soft_update_tau):
        with torch.no_grad():
            ...
            buf_value = [
                self.cri_target(buf_state[i:i + bs])
        ...
        for _ in range(int(buf_len / batch_size * repeat_times)):
            ...
            surrogate2 = advantage * ratio.clamp(1 - self.ratio_clip,
                                                 1 + self.ratio_clip)
    ...
    def get_reward_sum_gae(self, buf_len, ten_reward, ten_mask, ten_value):
        ...
        buf_advantage = torch.empty(buf_len,
                                    dtype=torch.float32,
        ...
        for i in range(buf_len - 1, -1, -1):
            ...
            pre_advantage = ten_value[
                i] + buf_advantage[i] * self.lambda_gae_adv
        return buf_r_sum, buf_advantage
...
class AgentDiscretePPO(AgentPPO):
    ...
    def explore_env(self, env, target_step):
        ...
        for i in range(target_step):
            ...
            trajectory_temp.append(
                (state, reward, done, a_int, a_prob))  # different
...
def train_and_evaluate(args, agent_id=0):
    ...
    def update_buffer(_trajectory):
        ...
        ten_reward = torch.as_tensor(_trajectory[1],
                                     dtype=torch.float32) * reward_scale
        ten_mask = (1.0 - torch.as_tensor(_trajectory[2], dtype=torch.float32)
                    ) * gamma  # _trajectory[2] = done
...
class Evaluator:
    ...
    def evaluate_and_save(self, act, steps, r_exp, log_tuple) -> bool:
        ...
        rewards_steps_list = [
            get_episode_return_and_step(self.env, act, self.device)
        ...
        if r_avg > self.r_max:  # save checkpoint with highest episode return
            ...
            torch.save(act.state_dict(),
                       act_save_path)  # save policy network in *.pth
            print(
...
class PreprocessEnv(gym.Wrapper):  # environment wrapper
    ...
    def __init__(self, env, if_print=True):
        ...
        (self.env_name, self.state_dim, self.action_dim, self.action_max,
         self.max_step, self.if_discrete,
         self.target_return) = get_gym_env_info(self.env, if_print)
    ...
    def step(self, action: np.ndarray):
        state, reward, done, info_dict = self.env.step(action *
                                                       self.action_max)
...
def get_gym_env_info(env, if_print):
    ...
    state_dim = state_shape[0] if len(
        state_shape) == 1 else state_shape  # sometimes state_dim is a list
    ...
    if if_discrete:  # make sure it is discrete action space
        ...
        action_max = int(1)
    elif isinstance(env.action_space,
                    gym.spaces.Box):  # make sure it is continuous action space
...
def demo_continuous_action():
    ...
    if if_train_pendulum:
        ...
        args.env = PreprocessEnv(
            env=gym.make('Pendulum-v1'))  # env='Pendulum-v0' is OK.
...
def evaluate_agent():
    ...
    actor.load_state_dict(
        torch.load(
            'D:\\workspace\\PycharmProjects\\RL\\AgentPPO_RequestEnvNoSim_1\\actor.pth'
        ))
